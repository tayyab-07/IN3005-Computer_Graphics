...
void Game::Render() 
{
	...
	pSphereProgram->SetUniform("light[0].position", viewMatrix*lightPosition1); // Light position in eye coordinates
	pSphereProgram->SetUniform("light[0].La", glm::vec3(1.0f, 0.0f, 1.0f));
	pSphereProgram->SetUniform("light[0].Ld", glm::vec3(1.0f, 0.0f, 1.0f));
	pSphereProgram->SetUniform("light[0].Ls", glm::vec3(1.0f, 0.0f, 1.0f));
	...
	pSphereProgram->SetUniform("light[0].direction", glm::normalize(viewNormalMatrix * glm::vec3(0, -1, 0)));
	pSphereProgram->SetUniform("light[0].exponent", 20.0f);
	pSphereProgram->SetUniform("light[0].cutoff", 30.0f);

	// Set light and materials in sphere programme
	glm::vec4 lightPosition2(50, 100, 50, 1); // Position of light source *in world coordinates*
	pSphereProgram->SetUniform("light[1].position", viewMatrix * lightPosition2); // Light position in eye coordinates
	pSphereProgram->SetUniform("light[1].La", glm::vec3(1.0f, 1.0f, 0.0f));
	pSphereProgram->SetUniform("light[1].Ld", glm::vec3(1.0f, 1.0f, 0.0f));
	pSphereProgram->SetUniform("light[1].Ls", glm::vec3(1.0f, 1.0f, 0.0f));
	pSphereProgram->SetUniform("light[1].direction", glm::normalize(viewNormalMatrix * glm::vec3(0, -1, 0)));
	pSphereProgram->SetUniform("light[1].exponent", 20.0f);
	pSphereProgram->SetUniform("light[1].cutoff", 30.0f);


	for (int x = -40; x < 60; x=x+10)
	{
		for (int z = -40; z < 60; z=z+10)
		{
			// Render the sphere
			modelViewMatrixStack.Push();
				modelViewMatrixStack.Translate(glm::vec3(x, 5.0f, z));
