...
void CTreeMesh::MeshEntry::Init(const std::vector<Vertex1>& Vertices,
    const std::vector<unsigned int>& Indices)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex1) * Vertices.size(), &Vertices[0], GL_STATIC_DRAW);
...
bool CTreeMesh::Load(const std::string& Filename)
{
...
bool CTreeMesh::InitFromScene(const aiScene* pScene, const std::string& Filename)
{
...
void CTreeMesh::InitMesh(unsigned int Index, const aiMesh* paiMesh)
{
    ...
    for (unsigned int i = 0; i < paiMesh->mNumVertices; i++) {
        ...
        Vertex1 v(glm::vec3(pPos->x, pPos->y, pPos->z),
...
bool CTreeMesh::InitMaterials(const aiScene* pScene, const std::string& Filename)
{
...
void CTreeMesh::Render()
{
    ...
    for (unsigned int i = 0; i < m_Entries.size(); i++) {
        ...
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex1), 0);
        glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex1), (const GLvoid*)12);
        glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex1), (const GLvoid*)20);
