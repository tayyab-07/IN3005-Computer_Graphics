...
void Game::Render() 
{
	...
	//Game::DrawTriangle(glm::vec3 (0, 5, 0));
	//Game::DrawTriangleStack(glm::vec3(0, 2, -5));

	for (int i = (-5); i <= 5; i=i+5)
	{
		Game::DrawTriangleStack(glm::vec3(i, 2, -5));
	}
...
{
	for (float i = 0; i < 11; i=i+m_spacing)
	{
		Game::DrawTriangle(glm::vec3(s.x, s.y, s.z + i));
